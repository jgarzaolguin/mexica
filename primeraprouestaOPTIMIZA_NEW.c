   ...
   index = 0;
           do {
               ...
               if (strcmp(using_gamma,"YES") == 0){
                   ...
                   rango_a[index_t] = valpha_opt[index_t] + 3.3;
...
//identifica la mejor condicion de kato cercana a 1 y mejor energia
 if (w == 1) { 
  ...
  if (expalpha_opt[j] - (double) 1.f/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 1.f/(1 + cut);
    valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
     else
  if (expalpha_opt[j] - (double) 0.9/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.9/(1 + cut);
    valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
    else
  if (expalpha_opt[j] - (double) 0.8/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.8/(1 + cut);
    valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
    else
  if (expalpha_opt[j] - (double) 0.7/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.7/(1 + cut);
    valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
    else
  if (expalpha_opt[j] - (double) 0.6/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.6/(1 + cut);
    valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
    else
   if (expalpha_opt[j] - (double) 0.5/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.5/(1 + cut);
     valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
     else
   if (expalpha_opt[j] - (double) 0.4/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.4/(1 + cut);
     valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
     else
   if (expalpha_opt[j] - (double) 0.3/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.3/(1 + cut);
     valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
     else
   if (expalpha_opt[j] - (double) 0.2/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.2/(1 + cut);
     valpha_opt[j] = expalpha_opt[j] - range_inter_a; 
  }
     else
   if (expalpha_opt[j] - (double) 0.1/(1 + cut) > restric_a[j]) {
    range_inter_a = (double) 0.1/(1 + cut);
     valpha_opt[j] = expalpha_opt[j] - range_inter_a;
  }
     else {
     range_inter_a = 0.1;
  ...
  if (expbeta_opt[j]  - (double) 1.f/(1 + cut) > 1.1) {
    range_inter_b = (double) 1.f/(1 + cut);
     vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
      else
  if (expbeta_opt[j]  - (double) 0.9/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.9/(1 + cut);
     vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
      else
  if (expbeta_opt[j]  - (double) 0.8/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.8/(1 + cut);
     vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
      else
  if (expbeta_opt[j]  - (double) 0.7/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.7/(1 + cut);
     vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
      else
  if (expbeta_opt[j]  - (double) 0.6/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.6/(1 + cut);
     vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
      else
   if (expbeta_opt[j]  - (double) 0.5/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.5/(1 + cut);
      vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b; 
  }
       else
   if (expbeta_opt[j]  - (double) 0.4/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.4/(1 + cut);
      vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
       else
   if (expbeta_opt[j]  - (double) 0.3/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.3/(1 + cut);
      vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
       else
   if (expbeta_opt[j]  - (double) 0.2/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.2/(1 + cut);
      vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
       else
   if (expbeta_opt[j]  - (double) 0.1/(1 + cut) > 1.1) {
    range_inter_b = (double) 0.1/(1 + cut);
      vbeta_opt[j]  = expbeta_opt[j]  - range_inter_b;
  }
       else {
        range_inter_b = 0.1;
    ...
    //mrb rango_b[j] = expbeta_opt[j] + (double) 1.0f/(1 + cut);
     rango_a[j] = expalpha_opt[j] + range_inter_a;
     rango_b[j] = expbeta_opt[j] + range_inter_b;
  ...
  iteraciones = 3000; //iteraciones/4 + iteraciones/2;
  if (w == 1)
  iteraciones = 1000; //iteraciones/4 + iteraciones/2;

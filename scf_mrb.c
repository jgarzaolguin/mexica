 ...
 *e_coul   = suma2;
 *e_exch   = suma3;
 *energia  = (sumak + sumav + suma2 + suma3);
...
{
 float          tiempo_usado;
 int i, j, compara, iter, iter_inter, dim, elec, valor, bandera;
...
extern int compute_energy(int     compara,
 ...
 extern int main_diis(int     nt,
                      ...
                      double *mat_temp_2,
                      double *mat_temp_3,
                      double *mat_e_store[800],
                      double *mat_f_store[800],
                      double *matfock);
...
//                        int* , double* , int* );
 ...
 extern double xc_energy(int     points,
                         int     compara,
                         char   **save_dft,
                         double *weight_dft,
                         int     flag_dft,
 ...
 double cx_D, 
        ...
        *grid_secder_beta, 
        *grid_der_beta, 
        *array_i, *array_ii, *array_iii, *array_iv;
...
//   else bandera_GPU = 0;
  ...
  for (i = 0; i < 100; i++) {
   ...
   if (strcmp(bound,"finite") == 0 || strcmp(bound,"dielectricc") == 0) {
     NC_minus = NULL;
...
//   bandera_GPU =   
 ...
 transforma(nt, mats, matx);
  ...
  ///////COMIENZA_DFT
   //mrb     if (flag_dft == 2 && strcmp(save_dft[1],"hf") == 0) {
   //mrb       e_xc = 0.f;
   //mrb     }
   //mrb     else {
   //mrb     grid_rhorad(z,
   ...
   //mrb                 n_points,
   //mrb                 save_i,
   //mrb                 arreglo_factorial,
   //mrb                 arreglo_inv_factorial);
   ...
   //mrb                    grid,
   //mrb                    grid_secder,
   //mrb                    grid_secder_beta,
   //mrb                    n_points,
   //mrb                    save_i,
   //mrb                    arreglo_factorial,
   //mrb                    arreglo_inv_factorial);

   //mrb       e_xc = xc_energy(n_points,
   //mrb                        compara,
   //mrb                        save_dft,
   //mrb                        weight_dft,
   //mrb                        flag_dft,
   ...
   //mrb     }
    ...
    //mrb  if (flag_dft != 2 && strcmp(save_dft[1],"hf") != 0) {
    //mrb     if(compara == 0) {
    //mrb       for (i = 0; i < cuad; i++) {
    //mrb          temp_xcmat = 0.f;
    //mrb          temp_xcmat = coef_HF*matg_exch[i] + mat_ks[i];
    //mrb          matg_exch[i] = temp_xcmat;
    //mrb       }
    //mrb      } else {
    //mrb             for (i = 0; i < cuad; i++) {
    //mrb                 temp_xcmat = 0.f;
    //mrb                 temp_xcmat = coef_HF*matg_exch_alfa[i] + mat_ks[i];
    //mrb                 matg_exch_alfa[i] = temp_xcmat;
    //mrb                 temp_xcmat = 0.f;
    //mrb                 temp_xcmat = coef_HF*matg_exch_beta[i] + mat_ks_beta[i];
    //mrb                 matg_exch_beta[i] = temp_xcmat;
    //mrb              }
    //mrb        }
    //mrb    }
            ...
            energiavieja = e_check;
             ...
             printf("Kinetic energy                       = %15.7f\n", e_kin);
             if (strcmp(bound,"finite") == 0){
             printf("Kinetic energy internal              = %15.7f\n", e_kinint);
             printf("Kinetic energy external              = %15.7f\n", e_kinext);
               }
             printf("Nuc-elec energy                      = %15.7f\n", e_v);
             printf("One-electron energy                  = %15.7f\n", e_core);
             printf("Coulomb energy                       = %15.7f\n", e_coul);
  //QUITAR DESPUES
             if (compara == 0) {
               if(strcmp(save_dft[1],"hf") == 0 && flag_dft == 2)
                 printf("Exchange energy                      = %15.7f\n", coef_HF*e_exch);
               else 
                 printf("Exchange-Correlation energy          = %15.7f\n", coef_HF*e_exch + e_xc);
             } else
               printf("Exchange energy                      = %15.7f\n", e_exch);
   
             printf("Hartree-Fock energy                  = %15.9f (%15.4f Ryd)\n", energia, 2.f*energia);
             printf("POT/KIN                              = %15.9f\n", (e_v + e_coul + e_exch)/e_kin);
              ...
              grid_derrad(z,
               ...
               if (print_vectors == 1) {
                 ...
                 printf("---------------------------\n");
                   ...
                   printf("\nNumber electrons from integral rho %5.4lf\n\n", SHAN);
...
/*compara*/} else {
               for (i = 0; i < nt; i++) 
                 printf("Eigenvalue %d: alpha | beta = %8.4f  %8.4f\n", i, valoresalfa[i], valoresbeta[i]);
               grid_rhorad(z,
                           using_gamma,
                           compara,
                           bound,

...
int xc_over_grid(int compara, char **save_dft, int flag_dft, double *weight_dft,  int points, double *grid, 
                 double *rho, double *der_rho) {
 int i, dft;
 double weight, pot_x, arg1, arg2, arg3;
 double varS[10000], dervarS[10000];
 extern void pbegrid(double *rho, double *varS, double *der_varS, double *pot_x);
 ...
 if (compara == 0) {
   for (dft = 1; dft < flag_dft; dft++) {
     ...
     if(strcmp(save_dft[dft], "pbe") == 0) {
       ...
       printf("jgo salgo varS\n");
       numerical_der(points, grid, varS, dervarS);
       for (i = 0; i < points; i++) {
         arg1 = rho[i];
         arg2 = varS[i];
         arg3 = dervarS[i];
         pbegrid_(&arg1, &arg2, &arg3, &pot_x);
         printf("%f %f %f %f\n", grid[i], varS[i], dervarS[i],
                 pot_x);
       }
     }
   } 
